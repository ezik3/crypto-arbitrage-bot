...
export class FlashLoanManager {
    ...
    private readonly AAVE_FLASH_LOAN_FEE = 0.0009; // 0.09%
    private readonly ETH_PRICE_USD = 2000;           // rough estimate; replace with live feed
     ...
     */
    private async findProfitableRoutes(token: string, amount: string): Promise<{
        profit: number;
        expectedReturn: number;
        path: string[];
    }[]> {
        const routes: { profit: number; expectedReturn: number; path: string[] }[] = [];

        const tokenAddress = TOKEN_ADDRESSES[token];
        if (!tokenAddress) return routes;

        // Skip on-chain calls if no RPC configured
        if (!process.env.ETH_RPC_URL) return routes;

        const amountIn = utils.parseUnits(amount, token === 'WBTC' ? 8 : 6).toString();
        ...
        for (const [dexName, routerAddr] of Object.entries(DEX_ROUTERS)) {
            try {
                const router = new Contract(routerAddr, UNI_V2_ROUTER_ABI, this.provider);
                const weth = TOKEN_ADDRESSES['WETH'];

                // Quote: token -> WETH
                const amountsOut: any[] = await router.getAmountsOut(amountIn, [tokenAddress, weth]);
                const wethOut = parseFloat(utils.formatEther(amountsOut[1]));
    ...
    private async calculateGasCostFromEstimate(gasEstimate: number): Promise<number> {
        try {
            ...
            const gasPrice = feeData.gasPrice ?? utils.parseUnits('30', 'gwei');
            const gasCostWei = gasPrice.mul(gasEstimate);
            return parseFloat(utils.formatEther(gasCostWei));
        } catch (error) {
